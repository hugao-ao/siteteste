// =============================================
// MÓDULO DE QUESTÕES PERTINENTES — CÓPIA v2 (diagnostico/)
// diagnostico/questoes-module.js
//
// Cópia fiel de questoes-module.js da raiz (a v1 continua usando o
// original, que NÃO muda). Esta cópia é carregada só pela página v2
// (diagnostico/diagnostico-financeiro.html) e acrescenta duas APIs de
// leitura para o relatório vivo do hub (relatorio-diagnostico.js):
//   window.getPlanoPorSecao()    → plano do gabarito, um tópico por seção
//   window.getQuestoesPorSecao() → perguntas e respostas agrupadas por seção
// Renderização, gabarito, responderQuestao e get/set continuam iguais.
// =============================================
...
(function() {
  ...
  // =============================================
  // API v2 — PLANO POR SEÇÃO (relatório vivo do hub)
  // Mesma lógica de gerarConteudoPlano() (fusões, contaComoNao, dado()),
  // só que sem achatar em lista: um tópico por regra.secao, indexado pelo
  // slug da seção. O fechamento sai separado e só existe quando há ao
  // menos um tópico. Nada aqui calcula análise — só declara entregas.
  // =============================================
  window.getPlanoPorSecao = function() {
    const dados = coletarDados();
    const regras = definirRegras(dados);
    const ehNao = contaComoNao;
    const porSecao = {};
    ...
    regras.forEach(regra => {
      ...
      if (itens.length > 0) {
        porSecao[regra.secao] = { titulo: regra.titulo, itens: itens };
        total += itens.length;
      }
    });

    const fechamento = total > 0
      ? ['Consolidar as frentes acima em cronograma, com responsáveis e prazos.']
      : [];

    return { porSecao: porSecao, fechamento: fechamento, total: total };
  };

  // =============================================
  // API v2 — QUESTÕES POR SEÇÃO (relatório vivo do hub)
  // { [secaoId]: [{ id, texto, resposta }] } — resposta '' quando em branco.
  // =============================================
  window.getQuestoesPorSecao = function() {
    const resultado = {};
    QUESTOES.forEach(q => {
      if (!resultado[q.secao]) resultado[q.secao] = [];
      resultado[q.secao].push({ id: q.id, texto: q.texto, resposta: respostaDe(q.id) });
    });
    return resultado;
  };
