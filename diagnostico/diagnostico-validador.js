// ========================================
// DIAGNÓSTICO VALIDADOR (v2) - Informações Faltantes para Ferramentas
// ========================================
// CÓPIA DA V2: este arquivo é a cópia editada de /diagnostico-validador.js (raiz)
// usada exclusivamente por /diagnostico/diagnostico-financeiro.html.
// A versão da raiz (v1) NÃO é tocada; toda mudança da v2 fica aqui.
//
...
//  - Juros & Dívidas: exige também 'situacao_divida' e 'proposta_em_vigor'.
//  - Fluxo de Caixa: exige também 'nivel_conforto' e 'alteravel' nas despesas manuais.
//  - Nova ferramenta 'Perfil Financeiro (automático)': lista os faltantes
//    informados pelo motor (window.getPerfilFinanceiroFaltantes, de perfil-financeiro.js).
//  - Timing: além do setTimeout de 2 s no DOMContentLoaded, re-renderiza ao ouvir
//    os eventos 'diagnostico:carregado' (800 ms depois) e 'diagnostico:salvo' (imediato).
...
(function() {
  ...
  const REQUISITOS = {
    juros: {
      ...
      campos: [
        ...
        // v2: campos usados pelo motor do perfil financeiro (camada de dívida)
        { campo: 'situacao_divida', label: 'Situação da dívida', obrigatorio: true },
        { campo: 'proposta_em_vigor', label: 'Proposta de renegociação em mãos?', obrigatorio: true }
      ]
    },
    fluxo: {
      nome: 'Fluxo de Caixa',
      ...
      campos_despesa: [
        ...
        // v2: campos usados pelo motor do perfil financeiro (camada de fluxo)
        { campo: 'nivel_conforto', label: 'Nível de Conforto (1-5)', obrigatorio: true },
        { campo: 'alteravel', label: 'É alterável?', obrigatorio: true }
      ],
      campos_receita: [
    ...
    // v2: os faltantes do perfil financeiro não são avaliados aqui por campo;
    // o motor (perfil-financeiro.js) já devolve a lista pronta em {item, campo}
    // via window.getPerfilFinanceiroFaltantes(). Por isso 'campos' fica vazio.
    perfil: {
      nome: 'Perfil Financeiro (automático)',
      icone: 'fa-user-tag',
      cor: '#d4af37',
      secao: 'perfil_financeiro',
      campos: []
    }
  };
  ...
  function validarDiagnostico() {
    ...
    // v2: o getter real devolve { objetivos: [...] } (não "normais"/"aposentadoria");
    // traduz para as chaves que REQUISITOS.oraculo espera, sem tocar na raiz.
    const objetivosBrutos = Array.isArray(objetivosData.objetivos) ? objetivosData.objetivos : [];
    const objetivosNormais = objetivosBrutos
      .filter(o => o && o.tipo !== 'aposentadoria' && o.tipo !== 'intangivel')
      .map(o => ({ nome: o.descricao, valor: o.meta_acumulo || o.valor_final, prazo: o.prazo_data ? 1 : o.prazo_meses }));
    const objApos = objetivosBrutos.find(o => o && o.tipo === 'aposentadoria');
    const aposentadoria = objApos
      ? { idade_aposentadoria: objApos.prazo_tipo === 'idade' ? objApos.prazo_idade : (objApos.prazo_meses || objApos.prazo_data ? 1 : 0), renda_desejada: objApos.renda_anual }
      : {};
    ...
    // 8. Validar Perfil Financeiro (automático) → motor perfil-financeiro.js (v2)
    // O motor devolve os itens já no formato do validador: [{ item, campo }].
    // Se o motor ainda não carregou (getter ausente), a seção é ignorada.
    const faltasPerfilBrutas = window.getPerfilFinanceiroFaltantes ? window.getPerfilFinanceiroFaltantes() : [];
    const faltasPerfil = Array.isArray(faltasPerfilBrutas) ? faltasPerfilBrutas : [];
    if (faltasPerfil.length) {
      faltantes.push({
        ferramenta: REQUISITOS.perfil,
        itens: faltasPerfil
      });
    }
  ...
  // v2: nomes de despesas/dívidas digitados pelo consultor entram no painel — escapar antes do innerHTML
  function esc(s) {
              ...
              <div style="display: grid; gap: 0.4rem;">
                ${agruparPorItem(grupo.itens).map(itemGrupo => `
                  <div style="padding: 0.5rem 0.8rem; background: rgba(255,255,255,0.03); border-radius: 6px;">
                    <strong style="color: var(--text-light, #ccc); font-size: 0.85rem;">${esc(itemGrupo.item)}:</strong>
                    <span style="color: var(--text-light, #aaa); font-size: 0.85rem;">
                      ${itemGrupo.campos.map(esc).join(', ')}
                    </span>
                  </div>
                `).join('')}
              </div>
  ...
  // v2: a página dispara 'diagnostico:carregado' quando o #main-content aparece
  // (dados já carregados nos módulos). Espera 800 ms para o motor do perfil
  // montar o card e calcular antes de listar os faltantes.
  document.addEventListener('diagnostico:carregado', function() {
    setTimeout(renderPainelFaltantes, 800);
  });

  // v2: a página dispara 'diagnostico:salvo' após o update bem-sucedido.
  // Re-renderiza na hora para refletir o que acabou de ser gravado.
  document.addEventListener('diagnostico:salvo', function() {
    renderPainelFaltantes();
  });
