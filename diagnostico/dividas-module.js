...
function updateDividaField(id, field, valor) {
  ...
  // v2 (Perfil Financeiro): parcela_proposta é monetária, mesmo padrão de valor_parcela
  const camposMonetarios = ['valor_inicial', 'valor_parcela', 'saldo_devedor', 'parcela_proposta'];
  ...
  } else if (field === 'inventariavel' || field === 'planejada' || field === 'divida_estruturada') {
...
function addDivida() {
  ...
  const novaDivida = {
    ...
    responsaveis: [],
    inventariavel: true,
    // v2 (Perfil Financeiro)
    situacao_divida: '',
    divida_estruturada: false,
    proposta_em_vigor: 'nao',
    parcela_proposta: 0
  };
    ...
    return `
      <div class="divida-card" data-divida-id="${divida.id}" style="background: var(--dark-bg); border: 2px solid var(--border-color); border-radius: 10px; padding: 1.2rem; margin-bottom: 1rem;">
        ...
        <div class="form-grid-3">
          ...
          <!-- Situação da dívida (Perfil Financeiro v2) -->
          <div class="form-group">
            <label for="divida_${divida.id}_situacao_divida">
              <i class="fas fa-traffic-light"></i> Situação da dívida
            </label>
            <select id="divida_${divida.id}_situacao_divida"
                    onchange="updateDividaField(${divida.id}, 'situacao_divida', this.value)">
              <option value="" ${!divida.situacao_divida ? 'selected' : ''}>Selecione...</option>
              <option value="em_dia" ${divida.situacao_divida === 'em_dia' ? 'selected' : ''}>Em dia</option>
              <option value="em_negociacao" ${divida.situacao_divida === 'em_negociacao' ? 'selected' : ''}>Em negociação</option>
              <option value="em_atraso" ${divida.situacao_divida === 'em_atraso' ? 'selected' : ''}>Em atraso (ignorada)</option>
              <option value="judicializada" ${divida.situacao_divida === 'judicializada' ? 'selected' : ''}>Judicializada</option>
              <option value="prescrita" ${divida.situacao_divida === 'prescrita' ? 'selected' : ''}>Prescrita</option>
            </select>
          </div>
          ...
          <!-- Proposta de renegociação em mãos? (Perfil Financeiro v2) -->
          <div class="form-group">
            <label for="divida_${divida.id}_proposta_em_vigor">
              <i class="fas fa-handshake"></i> Proposta de renegociação em mãos?
            </label>
            <select id="divida_${divida.id}_proposta_em_vigor"
                    onchange="updateDividaField(${divida.id}, 'proposta_em_vigor', this.value)">
              <option value="" ${!divida.proposta_em_vigor ? 'selected' : ''}>Selecione...</option>
              <option value="nao" ${divida.proposta_em_vigor === 'nao' ? 'selected' : ''}>Não</option>
              <option value="sim" ${divida.proposta_em_vigor === 'sim' ? 'selected' : ''}>Sim</option>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text"
                   id="divida_${divida.id}_parcela_proposta"
                   value="R$ ${formatarMoedaDivida(divida.parcela_proposta || 0)}"
                   oninput="updateDividaField(${divida.id}, 'parcela_proposta', this.value)"
                   onfocus="limparCampoDivida(this)"
                   placeholder="R$ 0,00">
          </div>
...
function setDividasData(data) {
  if (Array.isArray(data)) {
    // v2 (Perfil Financeiro): backfill dos defaults nas dívidas antigas — o spread preserva tudo que já existe
    dividas = data.map(d => ({ situacao_divida: '', divida_estruturada: false, proposta_em_vigor: 'nao', parcela_proposta: 0, ...d }));
    // Atualizar contador
    if (dividas.length > 0) {
