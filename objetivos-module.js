...
// Devolve o valor já gravado, para o onchange reexibir o campo normalizado
// sem precisar enxergar variaveisMercado (que é interna deste módulo).
      ...
      <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.6rem; padding-bottom: 0.6rem; border-bottom: 1px solid rgba(212, 175, 55, 0.2); flex-wrap: wrap;">
        ...
        <div style="display: flex; gap: 0.4rem; flex: 1; justify-content: flex-end; flex-wrap: wrap; align-items: center;">
          ...
          <span style="padding: 0.2rem 0.3rem; background: rgba(40, 167, 69, 0.1); border-radius: 4px; font-size: 0.7rem; display: flex; align-items: center; gap: 0.2rem;">
            ...
            <input type="text" inputmode="decimal" value="${formatarNumeroBR(variaveisMercado.cdi)}" onchange="this.value = formatarNumeroBR(updateVariavelMercado('cdi', parseNumeroBR(this.value)));" style="width: 42px; padding: 0.1rem 0.2rem; background: transparent; border: 1px solid rgba(40,167,69,0.3); border-radius: 3px; color: #28a745; font-size: 0.7rem; font-weight: 600; text-align: center;">%
          </span>
          <span style="padding: 0.2rem 0.3rem; background: rgba(220, 53, 69, 0.1); border-radius: 4px; font-size: 0.7rem; display: flex; align-items: center; gap: 0.2rem;">
            ...
            <input type="text" inputmode="decimal" value="${formatarNumeroBR(variaveisMercado.ipca)}" onchange="this.value = formatarNumeroBR(updateVariavelMercado('ipca', parseNumeroBR(this.value)));" style="width: 42px; padding: 0.1rem 0.2rem; background: transparent; border: 1px solid rgba(220,53,69,0.3); border-radius: 3px; color: #dc3545; font-size: 0.7rem; font-weight: 600; text-align: center;">%
          </span>
          <span style="padding: 0.2rem 0.3rem; background: rgba(23, 162, 184, 0.1); border-radius: 4px; font-size: 0.7rem; display: flex; align-items: center; gap: 0.2rem;">
            ...
            <input type="text" inputmode="decimal" value="${formatarNumeroBR(variaveisMercado.rent_anual_aposentadoria)}" onchange="this.value = formatarNumeroBR(updateVariavelMercado('rent_anual_aposentadoria', parseNumeroBR(this.value)));" style="width: 42px; padding: 0.1rem 0.2rem; background: transparent; border: 1px solid rgba(23,162,184,0.3); border-radius: 3px; color: #17a2b8; font-size: 0.7rem; font-weight: 600; text-align: center;">%
          </span>
          <span style="padding: 0.2rem 0.3rem; background: rgba(76, 175, 80, 0.1); border-radius: 4px; font-size: 0.7rem; display: flex; align-items: center; gap: 0.2rem;">
            ...
            <input type="text" inputmode="decimal" value="${variaveisMercado.dolar ? formatarNumeroBR(variaveisMercado.dolar, 2) : ''}" onchange="var d = updateVariavelMercado('dolar', parseNumeroBR(this.value)); this.value = d ? formatarNumeroBR(d, 2) : '';" style="width: 42px; padding: 0.1rem 0.2rem; background: transparent; border: 1px solid rgba(76,175,80,0.3); border-radius: 3px; color: #4CAF50; font-size: 0.7rem; font-weight: 600; text-align: center;">
          </span>
        </div>
      </div>
...
function renderCardAposentadoria(obj, pessoas, patrimonioAposentadoriaPorPessoa) {
  ...
  // Calcular prazo baseado no prazo_tipo usando data_reuniao como referência
  const mesesRestantesItem = calcularMesesRestantesObj(obj, pessoas);
  const anosRestantes = Math.max(0, Math.round(mesesRestantesItem / 12));
    ...
    <div style="margin-top: 2rem; padding-top: 1.5rem; border-top: 2px solid rgba(212, 175, 55, 0.3);">
      ...
      <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
        <div style="background: var(--card-bg); border: 1px solid var(--accent-color); border-radius: 8px; padding: 1rem; flex: 1; min-width: 300px;">
          ...
          ${renderResumoAnalise(simulacao, objetivosAposentadoria, objetivosNormais, perfilAnaliseSelecionado)}
        ...
        <div style="background: var(--card-bg); border: 1px solid ${coresComparativas[idx] || CORES_PERFIS[(idx+1) % CORES_PERFIS.length]}; border-radius: 8px; padding: 1rem; flex: 1; min-width: 300px;">
          ...
          ${renderResumoAnalise(comp.simulacao, objetivosAposentadoria, objetivosNormais, comp.perfilId)}
        </div>
        `).join('')}
      </div>
    </div>
...
function simularEvolucaoPatrimonial(aposentadorias, objetivosNormais, perfilIdOverride) {
  ...
  // Determinar horizonte total (até a data mais distante entre TODOS os itens)
  let maxMeses = 12; // mínimo 1 ano
  ...
  // Incluir intangíveis se existirem
  const todosObjetivos = objetivos || [];
  todosObjetivos.filter(o => o.tipo === 'intangivel').forEach(obj => {
    const meses = calcularMesesRestantesObjNormal(obj);
    if (meses > maxMeses) maxMeses = meses;
  });
...
function renderResumoAnalise(simulacao, aposentadorias, objetivosNormais, perfilIdAnalise) {
  const pessoas = getPessoasDisponiveis();
  const perfilId = perfilIdAnalise || getPerfilAnalise();
  let html = '';
  ...
  // Somatório de aportes mensais necessários com memória de cálculo
  let totalAporteMensal = 0;
  const memoriaCalculo = [];
  const rentAnualPerfil = getRentabilidadePorPerfil(perfilId);
  ...
  aposentadorias.forEach(obj => {
    ...
    const meses = calcularMesesRestantesObj(obj, pessoas);
    const aporte = calcularAporteMensalNecessario(capitalNec, obj.valor_inicial || 0, meses, perfilId);
    totalAporteMensal += aporte;
    if (capitalNec > 0) {
      memoriaCalculo.push({
        tipo: 'aposentadoria',
        descricao: obj.descricao || 'Aposentadoria',
        meta: capitalNec,
        valorInicial: obj.valor_inicial || 0,
        meses: meses,
        aporteMensal: aporte
      });
    }
  ...
  objetivosNormais.forEach(obj => {
    ...
    const meses = calcularMesesRestantesObjNormal(obj);
    const aporte = calcularAporteMensalNecessario(meta, obj.valor_inicial || 0, meses, perfilId);
    totalAporteMensal += aporte;
    if (meta > 0) {
      memoriaCalculo.push({
        tipo: 'objetivo',
  ...
  if (totalAporteMensal > 0 || memoriaCalculo.length > 0) {
    ...
    let memoriaHTML = '';
    memoriaCalculo.forEach(item => {
      const anos = (item.meses / 12).toFixed(1);
      memoriaHTML += `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.2rem 0; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 0.65rem;">
          <div style="flex: 1; color: var(--text-light);">
            <i class="fas fa-${item.tipo === 'aposentadoria' ? 'user' : 'bullseye'}" style="color: ${item.tipo === 'aposentadoria' ? '#28a745' : 'var(--accent-color)'};"></i>
            ${item.descricao}
          </div>
          <div style="color: var(--text-light); font-size: 0.6rem; text-align: right; min-width: 180px;">
            Meta: ${formatarMoedaObj(item.meta)} | Ini: ${formatarMoedaObj(item.valorInicial)} | ${anos}a
          </div>
          <div style="min-width: 90px; text-align: right; font-weight: 600; color: ${suficiente ? '#28a745' : '#dc3545'};">
            ${formatarMoedaObj(item.aporteMensal)}/mês
          </div>
        </div>
      `;
    });
    ...
    html += `
      <div style="margin-top: 0.8rem; padding: 0.6rem; background: rgba(${suficiente ? '40, 167, 69' : '220, 53, 69'}, 0.1); border-radius: 6px; border: 1px solid ${suficiente ? '#28a745' : '#dc3545'};">
        <div style="font-size: 0.7rem; color: var(--text-light); margin-bottom: 0.4rem; font-weight: 600;">
          <i class="fas fa-calculator"></i> Memória de Cálculo - Aportes (Rent. ${rentAnualPerfil.toFixed(2)}% a.a. = ${(Math.pow(1 + rentAnualPerfil/100, 1/12) - 1).toFixed(4).replace('0.', '0,')}% a.m.)
        </div>
        <div style="margin-bottom: 0.5rem;">
          ${memoriaHTML}
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.5rem; padding-top: 0.4rem; border-top: 1px solid rgba(255,255,255,0.1);">
          <div>
            <div style="font-size: 0.65rem; color: var(--text-light);"><i class="fas fa-coins"></i> Total Necessário:</div>
            <div style="font-size: 0.95rem; font-weight: 600; color: ${suficiente ? '#28a745' : '#dc3545'};">${formatarMoedaObj(totalAporteMensal)}/mês</div>
          </div>
          <div>
            <div style="font-size: 0.65rem; color: var(--text-light);"><i class="fas fa-piggy-bank"></i> Disponível:</div>
            <div style="font-size: 0.95rem; font-weight: 600; color: ${suficiente ? '#28a745' : '#dc3545'};">${formatarMoedaObj(aporteDisponivel)}/mês</div>
          </div>
          <div style="text-align: center;">
            <div style="font-size: 0.65rem; color: var(--text-light);">Status:</div>
            <div style="font-size: 0.8rem; font-weight: 600; color: ${suficiente ? '#28a745' : '#dc3545'};">
