...
class ObrasManager {
    ...
    async renderProducoesDiarias() {
        ...
        for (const producao of this.producoesDiarias) {
            ...
            let integrantesEnvolvidos = [];

            if (producao.tipo_responsavel === 'equipe') {
                // Buscar equipe pelo número
                const equipe = this.equipesIntegrantes.find(item => 
                    item.id === producao.responsavel_id && item.tipo === 'equipe'
                );
                
                if (equipe) {
                    nomeResponsavel = equipe.nome; // Já está usando o número da equipe
                    
                    // Buscar todos os integrantes desta equipe
                    try {
                        const { data: integrantesEquipe, error } = await supabaseClient
                            .from('integrantes_hvc')
                            .select('id, nome')
                            .eq('equipe_id', producao.responsavel_id)
                            .eq('ativo', true);
                        
                        if (!error && integrantesEquipe) {
                            integrantesEnvolvidos = integrantesEquipe.map(i => i.nome);
                        }
                    } catch (error) {
                ...
                const integrante = this.equipesIntegrantes.find(item => 
                    item.id === producao.responsavel_id && item.tipo === 'integrante'
                );
                
                if (integrante) {
                    nomeResponsavel = integrante.nome;
                    integrantesEnvolvidos = [integrante.nome];
                }
            }
            ...
            // Obter detalhes dos serviços executados
            const quantidadesObj = producao.quantidades_servicos || {};
            const servicosExecutados = [];
            
            Object.keys(quantidadesObj).forEach(servicoId => {
                const servico = this.servicosObra.find(s => s.id.toString() === servicoId);
                if (servico) {
                    servicosExecutados.push({
                        nome: servico.descricao,
                        quantidade: quantidadesObj[servicoId],
                        unidade: servico.unidade
                    });
                }
            });

            const totalServicos = servicosExecutados.length;
            ...
            producaoDiv.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: start; gap: 1rem;">
                    <div style="flex: 1;">
                        ...
                        ${integrantesEnvolvidos.length > 0 ? `
                            <div style="color: #b0b0b0; font-size: 0.85em; margin-bottom: 0.5rem;">
                                <i class="fas fa-user-friends"></i>
                                <strong>Integrantes:</strong> ${integrantesEnvolvidos.join(', ')}
                            </div>
                        ` : ''}
                        
                        <div style="color: #c0c0c0; font-size: 0.9em; margin-bottom: 0.5rem;">
                            <i class="fas fa-tools"></i>
                            <strong>${totalServicos} serviço(s) executado(s):</strong>
                        </div>
                        
                        ${servicosExecutados.length > 0 ? `
                            <div style="margin-left: 1.5rem; margin-bottom: 0.5rem;">
                                ${servicosExecutados.map(s => `
                                    <div style="color: #90EE90; font-size: 0.85em; margin-bottom: 0.2rem;">
                                        • ${s.nome}: <strong>${s.quantidade} ${s.unidade}</strong>
                                    </div>
                                `).join('')}
                            </div>
                        ` : ''}
    ...
    async filtrarProducoes() {
        ...
        if (filtroEquipe) {
            ...
            if (tipo === 'equipe') {
                // Filtrar por equipe: incluir todas as produções desta equipe
                producoesFiltradas = producoesFiltradas.filter(p => 
                    p.tipo_responsavel === 'equipe' && p.responsavel_id.toString() === id
                );
            } else if (tipo === 'integrante') {
                // Filtrar por integrante: incluir produções onde ele é responsável individual
                // OU onde ele faz parte da equipe responsável
                const integranteId = parseInt(id);
                
                const producoesFiltradas2 = [];
                
                for (const producao of producoesFiltradas) {
                    let incluir = false;
                    
                    if (producao.tipo_responsavel === 'integrante' && producao.responsavel_id === integranteId) {
                        // Integrante é responsável individual
                        incluir = true;
                    } else if (producao.tipo_responsavel === 'equipe') {
                        // Verificar se o integrante faz parte desta equipe
                        try {
                            const { data: integrantesEquipe, error } = await supabaseClient
                                .from('integrantes_hvc')
                                .select('id')
                            ...
                            if (!error && integrantesEquipe && integrantesEquipe.length > 0) {
                                incluir = true;
                            }
                        } catch (error) {
                            console.error('Erro ao verificar integrante na equipe:', error);
                        }
                    }
                    
                    if (incluir) {
                        producoesFiltradas2.push(producao);
                    }
                }
                
                producoesFiltradas = producoesFiltradas2;
            }
        }
        ...
        await this.renderProducoesDiarias();
        this.producoesDiarias = producaoOriginal;
    }
    
    async limparFiltrosProducao() {
