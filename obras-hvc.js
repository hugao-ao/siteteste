...
class ObrasManager {
    ...
    clearFormProducao() {
        ...
        // ✅ NOVO: Limpar container de ausências
        const ausenciasContainer = document.getElementById('ausencias-container');
        if (ausenciasContainer) {
            ausenciasContainer.innerHTML = '';
            ausenciasContainer.style.display = 'none';
        }
    }
                ...
                if (selectEquipe) {
                    ...
                    // ✅ NOVO: Adicionar listener para mostrar ausências quando equipe é selecionada
                    selectEquipe.removeEventListener('change', this.handleEquipeChange);
                    this.handleEquipeChange = (e) => this.mostrarAusenciasEquipe(e.target.value);
                    selectEquipe.addEventListener('change', this.handleEquipeChange);
    ...
    // ✅ NOVO: Função para mostrar checkboxes de ausências quando uma equipe é selecionada
    mostrarAusenciasEquipe(valor) {
        const ausenciasContainer = document.getElementById('ausencias-container');
        if (!ausenciasContainer) return;
        
        // Se não selecionou nada ou selecionou integrante individual, esconder
        if (!valor || !valor.startsWith('equipe:')) {
            ausenciasContainer.innerHTML = '';
            ausenciasContainer.style.display = 'none';
            return;
        }
        
        // Extrair ID da equipe
        const equipeId = valor.split(':')[1];
        
        // Buscar integrantes desta equipe
        const integrantesDaEquipe = this.equipesIntegrantes.filter(item => 
            item.tipo === 'integrante' && String(item.equipe_id) === String(equipeId)
        );
        
        if (integrantesDaEquipe.length === 0) {
            ausenciasContainer.innerHTML = '<p style="color: #ffc107; font-size: 0.9em;">Nenhum integrante cadastrado nesta equipe.</p>';
            ausenciasContainer.style.display = 'block';
            return;
        }
        
        // Criar checkboxes para marcar ausências
        let html = `
            <div style="margin-top: 1rem; padding: 1rem; background: rgba(255, 152, 0, 0.1); border: 1px solid rgba(255, 152, 0, 0.3); border-radius: 8px;">
                <label style="color: #ffc107; font-weight: bold; display: block; margin-bottom: 0.5rem;">
                    <i class="fas fa-user-slash"></i> Marcar Integrantes Ausentes:
                </label>
                <div style="display: flex; flex-wrap: wrap; gap: 0.5rem;">
        `;
        
        integrantesDaEquipe.forEach(integrante => {
            html += `
                <label style="display: flex; align-items: center; gap: 0.3rem; padding: 0.3rem 0.6rem; background: rgba(255, 255, 255, 0.05); border-radius: 4px; cursor: pointer; font-size: 0.9em;">
                    <input type="checkbox" class="ausencia-checkbox" value="${integrante.id}" style="cursor: pointer;">
                    <span style="color: #e0e0e0;">${integrante.nome}</span>
                </label>
            `;
        });
        ...
        html += `
                ...
                <small style="color: #a0a0a0; display: block; margin-top: 0.5rem;">Marque os integrantes que NÃO participaram desta produção.</small>
            </div>
        `;
        
        ausenciasContainer.innerHTML = html;
        ausenciasContainer.style.display = 'block';
    }
   ...
   async loadEquipesIntegrantes() {
    ...
    async handleSubmitProducao(e) {
        ...
        try {
            ...
            // ✅ NOVO: Coletar integrantes ausentes (apenas se for equipe)
            const integrantesAusentes = [];
            if (tipo === 'equipe') {
                const checkboxes = document.querySelectorAll('.ausencia-checkbox:checked');
                checkboxes.forEach(cb => integrantesAusentes.push(cb.value));
            }
            ...
            const producaoData = {
                ...
                integrantes_ausentes: integrantesAusentes // ✅ NOVO: Array de IDs dos ausentes
            };
    ...
    async loadProducaoData(producaoId) {
        try {
            ...
            // ✅ NOVO: Mostrar checkboxes de ausências se for equipe
            if (data.tipo_responsavel === 'equipe') {
                this.mostrarAusenciasEquipe(`equipe:${data.responsavel_id}`);
                
                // Marcar os integrantes ausentes
                const ausentes = data.integrantes_ausentes || [];
                setTimeout(() => {
                    ausentes.forEach(integranteId => {
                        const checkbox = document.querySelector(`.ausencia-checkbox[value="${integranteId}"]`);
    ...
    async visualizarRelatorio() {
        ...
        try {
            ...
            const { data: despesasObra } = await supabaseClient
                ...
                .select('*')
                .eq('tipo', 'pagamento')
                .eq('categoria', (obra.numero_obra || '').trim());
