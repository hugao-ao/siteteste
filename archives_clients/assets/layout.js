...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // 5. Botão Voltar ao Gerenciamento (apenas para equipe em modo cliente)
        const voltarBtn = document.getElementById('voltar-gerenciamento-btn');
    ...
    // === PLANO DE REFERÊNCIA COMO POP-UP (window.open) ===
    var _planoPopup = null;
    function openPlanoFlutuante(basePath) {
        // Se o popup já está aberto e não foi fechado, apenas dar foco
        if (_planoPopup && !_planoPopup.closed) {
            _planoPopup.focus();
            return;
        }
        // Abrir em nova janela popup (pode ser arrastada para outro monitor)
        var url = basePath + 'plano.html?embed=1';
        var w = 750, h = 600;
        var left = window.screenX + Math.round((window.outerWidth - w) / 2);
        var top = window.screenY + Math.round((window.outerHeight - h) / 2);
        _planoPopup = window.open(
            url,
            'PlanoReferencia',
