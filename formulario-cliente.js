...
;
    ...
    if (addPatrimonioBtn) {
        addPatrimonioBtn.addEventListener("click", () => {
            ...
            newPatrimonioEntry.dataset.patrimonioIndex = patrimonioIndex; 
...
;
            ...
            newPatrimonioEntry.innerHTML = `
                ...
                <div class="form-group" style="margin-top:0.5rem; margin-bottom:0.5rem;">
                    ...
                    <select name="patrimonio_pessoa_associada_${patrimonioIndex}" id="${selectPessoaHTMLId}" required>
                        ${optionsHTML}
                    </select>
                </div>
...
input[name="patrimonio_valor"]
            ...
            newPatrimonioEntry.querySelector(".btn-remove-item").addEventListener("click", () => {
                ...
                document.querySelectorAll("#patrimonio-list .item-entry").forEach((entry, i) => {
                    entry.dataset.patrimonioIndex = i;
...
_
                        ...
                        const newName = `${nameParts[0]}_${nameParts[1]}_${i}`;
                        radio.name = newName;
                        radio.id = `${newName}_${radio.value === 
sim
 ? 
sim
 : 
nao
                        ...
                        const label = entry.querySelector(`label[for^=\"${nameParts[0]}_${nameParts[1]}_${nameParts[2]}\"]`);
                        if (label) label.htmlFor = radio.id;
                    });
                    const selectPessoa = entry.querySelector(`select[name^=\"patrimonio_pessoa_associada_\"]`);
                    if (selectPessoa) {
                        selectPessoa.name = `patrimonio_pessoa_associada_${i}`;
                        selectPessoa.id = `patrimonio_pessoa_associada_${i}`;
                    }
                });
                updateDynamicFormSections();
            });
...
input[type="radio"]
                ...
                if (dadosFormulario.possui_patrimonio_fisico === "sim") {
                    document.querySelectorAll("#patrimonio-list .item-entry").forEach((entry) => {
                        const index = entry.dataset.patrimonioIndex; 
...
input[name="patrimonio_valor"]
                        ...
                        const seguroSim = entry.querySelector(`#patrimonio_seguro_sim_${index}`);
                        const seguro = seguroSim && seguroSim.checked ? "sim" : (entry.querySelector(`#patrimonio_seguro_nao_${index}`)?.checked ? "nao" : null);

                        const quitadoSim = entry.querySelector(`#patrimonio_quitado_sim_${index}`);
                        const quitado = quitadoSim && quitadoSim.checked ? "sim" : (entry.querySelector(`#patrimonio_quitado_nao_${index}`)?.checked ? "nao" : null);
...
;
                ...
                const seguroRadio = lastPatrimonioEntry.querySelector(`input[name="patrimonio_seguro_${currentIndex}"][value="${pat.possui_seguro}"]`);
                if (seguroRadio) seguroRadio.checked = true;

                const quitadoRadio = lastPatrimonioEntry.querySelector(`input[name="patrimonio_quitado_${currentIndex}"][value="${pat.esta_quitado}"]`);
                if (quitadoRadio) quitadoRadio.checked = true;

                if (pat.pessoa_associada) {
                    const selectPessoa = lastPatrimonioEntry.querySelector(`select[name=\"patrimonio_pessoa_associada_${currentIndex}\"]`);
                    if (selectPessoa) {
                        selectPessoa.value = pat.pessoa_associada;
                         if (!selectPessoa.value && pat.pessoa_associada) { 
                            console.warn(`Pessoa associada "${pat.pessoa_associada}" não encontrada no dropdown para patrimônio ${currentIndex}.`);
                        }
                    }
                }
            }
