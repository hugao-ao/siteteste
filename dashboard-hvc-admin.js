...
class DashboardHVC {
    ...
    async carregarProdutividadeIntegrantes() {
        ...
        // Processar produções EM EQUIPE (atribuir a cada integrante da equipe, DIVIDINDO pelo número de membros PRESENTES)
        producoesEquipes.forEach(prod => {
            const equipeId = String(prod.responsavel_id);
            
            // ✅ NOVO: Obter lista de integrantes ausentes desta produção
            const integrantesAusentes = prod.integrantes_ausentes || [];
            
            // Obter número total de membros da equipe
            const numMembrosTotal = equipeMembrosMap[equipeId] || 1;
            
            // ✅ NOVO: Calcular número de membros PRESENTES (total - ausentes)
            const numMembrosPresentes = Math.max(1, numMembrosTotal - integrantesAusentes.length);
            ...
            (integrantes || []).forEach(int => {
                ...
                // ✅ NOVO: Verificar se o integrante estava AUSENTE nesta produção
                const estaAusente = integrantesAusentes.includes(intId) || integrantesAusentes.includes(int.id);
                
                if (equipesDoIntegrante.has(equipeId) && !estaAusente) {
                    // Este integrante faz parte desta equipe E estava PRESENTE
                    ...
                    const producaoInfo = {
                        ...
                        numMembrosEquipe: numMembrosPresentes, // ✅ ATUALIZADO: Usar membros presentes
                        numMembrosTotal: numMembrosTotal, // Guardar total para referência
                        integrantesAusentes: integrantesAusentes.length, // Quantidade de ausentes
                        servicos: []
                    };

                    // Processar quantidades_servicos - DIVIDIR pelo número de membros PRESENTES
                    if (prod.quantidades_servicos && typeof prod.quantidades_servicos === 'object') {
                        Object.entries(prod.quantidades_servicos).forEach(([itemId, quantidade]) => {
                            ...
                            // ✅ ATUALIZADO: DIVIDIR quantidade e valor pelo número de membros PRESENTES
                            const qtdProporcional = qtdTotal / numMembrosPresentes;
                            const valorProporcional = (qtdTotal * precoUnitario) / numMembrosPresentes;
                            ...
                            producaoInfo.servicos.push({
                                ...
                                numMembros: numMembrosPresentes, // ✅ ATUALIZADO: Membros presentes
                                numMembrosTotal: numMembrosTotal // Total de membros da equipe
                            });
                            ...
                            if (servicoId) {
                                ...
                                produtividadeMap[intId].servicosEmEquipe[servicoId].producoes.push({
                                    ...
                                    numMembros: numMembrosPresentes, // ✅ ATUALIZADO: Membros presentes
                                    numMembrosTotal: numMembrosTotal, // Total de membros
                                    integrantesAusentes: integrantesAusentes.length // Quantidade de ausentes
                                });
                            }
                        });
